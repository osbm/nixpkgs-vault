---
aliases:
  - luajitPackages.luazip
tags:
  - license/unknown
  - outputs/out
---

# luajitPackages.luazip

## 📝 Description

Library for reading files inside zip files

## 📋 Package Information

- **Name**: `luajitPackages.luazip`
- **Version**: `1.2.7-1`
- **Available**: ✅ Yes
- **Broken**: ✅ No
- **Description**: Library for reading files inside zip files
- **Homepage**: [https://github.com/mpeterv/luazip](https://github.com/mpeterv/luazip)
- **License**: `unknown`
- **Platforms**: `aarch64-linux`, `armv5tel-linux`, `armv6l-linux`, `armv7a-linux`, `armv7l-linux`, `i686-linux`, `loongarch64-linux`, `m68k-linux`, `microblaze-linux`, `microblazeel-linux`, `mips-linux`, `mips64-linux`, `mips64el-linux`, `mipsel-linux`, `powerpc-linux`, `powerpc64-linux`, `powerpc64le-linux`, `riscv32-linux`, `riscv64-linux`, `s390-linux`, `s390x-linux`, `x86_64-linux`, `x86_64-darwin`, `aarch64-darwin`

## 🔧 Build Information

- **Derivation Path**: `/nix/store/9nmxcdjkpy9dxjnwgjp94w4viqd7y37z-luajit2.1-luazip-1.2.7-1.drv`
- **Source Position**: `/nix/store/ns30sqxb36k8jrds8z18rv96bpnwc60d-source/pkgs/development/lua-modules/generated-packages.nix:3423`
- **Outputs**:
  - `out`:  `/nix/store/9nmxcdjkpy9dxjnwgjp94w4viqd7y37z-luajit2.1-luazip-1.2.7-1`

## 🔗 Dependencies

- [[4685kbpkijviqi99s5gxkfpg4ss5ch32-source]]
- [[bjsb6wdjykafnkixq156qdvmxhsm2bai-bash-5.3p3]]
- [[bya8qs2138a0dndg8yq4gjn4vm85p3jg-luarocks_bootstrap-3.12.2]]
- [[cvjdzfcg0md0qsdx35yl2hm56zy80ma7-zziplib-0.13.80]]
- [[f8cy03ybcfqydagg2g6qiz46ddgcdwwq-wrap-lua-hook]]
- [[g6lq8rpn7m8l7srbkxf590mf0y2df4c7-luazip-1.2.7-1.rockspec]]
- [[m3m201807xp59vwfavxsqkskwxdlwggf-luazip-luarocks-config.lua]]
- [[p76r0cwlf6k97ibprrpfd8xw0r8wc3nx-stdenv-linux]]
- [[q1vkgklakdylvgjy1mw1f2k7c0r1h3pi-luajit-2.1.1741730670]]

## 📁 Input Sources

- `/nix/store/l622p70vy8k5sh7y5wizi5f2mic6ynpg-source-stdenv.sh`
- `/nix/store/shkw4qm9qcw5sc5n1k5jznc83ny02r39-default-builder.sh`

---
*Generated on 2025-09-27 12:03:55 UTC*
