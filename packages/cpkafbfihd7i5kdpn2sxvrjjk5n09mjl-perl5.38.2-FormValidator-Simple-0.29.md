---
aliases:
  - perl538Packages.FormValidatorSimple
tags:
  - license/unknown
  - outputs/devdoc
  - outputs/out
---

# perl538Packages.FormValidatorSimple

## 📝 Description

Validation with simple chains of constraints

## 📋 Package Information

- **Name**: `perl538Packages.FormValidatorSimple`
- **Version**: `0.29`
- **Available**: ✅ Yes
- **Broken**: ✅ No
- **Description**: Validation with simple chains of constraints
- **Homepage**: [https://metacpan.org/dist/FormValidator-Simple](https://metacpan.org/dist/FormValidator-Simple)
- **License**: `unknown`
- **Platforms**: `i686-cygwin`, `x86_64-cygwin`, `x86_64-darwin`, `aarch64-darwin`, `i686-freebsd`, `x86_64-freebsd`, `aarch64-freebsd`, `aarch64-genode`, `i686-genode`, `x86_64-genode`, `x86_64-solaris`, `javascript-ghcjs`, `aarch64-linux`, `armv5tel-linux`, `armv6l-linux`, `armv7a-linux`, `armv7l-linux`, `i686-linux`, `loongarch64-linux`, `m68k-linux`, `microblaze-linux`, `microblazeel-linux`, `mips-linux`, `mips64-linux`, `mips64el-linux`, `mipsel-linux`, `powerpc-linux`, `powerpc64-linux`, `powerpc64le-linux`, `riscv32-linux`, `riscv64-linux`, `s390-linux`, `s390x-linux`, `x86_64-linux`, `mmix-mmixware`, `aarch64-netbsd`, `armv6l-netbsd`, `armv7a-netbsd`, `armv7l-netbsd`, `i686-netbsd`, `m68k-netbsd`, `mipsel-netbsd`, `powerpc-netbsd`, `riscv32-netbsd`, `riscv64-netbsd`, `x86_64-netbsd`, `aarch64_be-none`, `aarch64-none`, `arm-none`, `armv6l-none`, `avr-none`, `i686-none`, `microblaze-none`, `microblazeel-none`, `mips-none`, `mips64-none`, `msp430-none`, `or1k-none`, `m68k-none`, `powerpc-none`, `powerpcle-none`, `riscv32-none`, `riscv64-none`, `rx-none`, `s390-none`, `s390x-none`, `vc4-none`, `x86_64-none`, `i686-openbsd`, `x86_64-openbsd`, `x86_64-redox`, `wasm64-wasi`, `wasm32-wasi`, `aarch64-windows`, `x86_64-windows`, `i686-windows`

## 🔧 Build Information

- **Derivation Path**: `/nix/store/cpkafbfihd7i5kdpn2sxvrjjk5n09mjl-perl5.38.2-FormValidator-Simple-0.29.drv`
- **Source Position**: `/nix/store/ns30sqxb36k8jrds8z18rv96bpnwc60d-source/pkgs/top-level/perl-packages.nix:14328`
- **Outputs**:
  - `devdoc`:  `/nix/store/cpkafbfihd7i5kdpn2sxvrjjk5n09mjl-perl5.38.2-FormValidator-Simple-0.29`
  - `out`:  `/nix/store/cpkafbfihd7i5kdpn2sxvrjjk5n09mjl-perl5.38.2-FormValidator-Simple-0.29`

## 🔗 Dependencies

- [[4w6dzhzmzn34hravxgb351mkbbpi5n5k-perl-5.38.2]]
- [[4z9j67bfbx4jy5xm34gqpm5vlz9ia3n9-perl5.38.2-List-MoreUtils-0.430]]
- [[74wpysx05013sgmhiiprmdiq2d53c3ry-perl5.38.2-CGI-4.59]]
- [[bjsb6wdjykafnkixq156qdvmxhsm2bai-bash-5.3p3]]
- [[bnp1bwncpi302szwhr8f5ymhp7fssj9c-perl5.38.2-Tie-IxHash-1.23]]
- [[hywwxgpg5a8ay5a8jnvbv7xjllgpkfp5-perl5.38.2-DateTime-Format-Strptime-1.79]]
- [[iasxd2aarcygzyrsw8l6mdri7y18s1qp-perl5.38.2-Date-Calc-6.4]]
- [[p76r0cwlf6k97ibprrpfd8xw0r8wc3nx-stdenv-linux]]
- [[r0gsmq9lbiw6p4dkllw0bx3k2dkns6sb-perl5.38.2-Class-Accessor-0.51]]
- [[rx9syqn0ip9sg29zvriwdl5sz8nbl2f3-perl5.38.2-UNIVERSAL-require-0.19]]
- [[wj9pliswh19ck8yw5z8w584qj5agsl6b-FormValidator-Simple-0.29.tar.gz]]
- [[xy3hyn7k968fksr433yqxiigmri6gabw-perl5.38.2-Class-Data-Accessor-0.04004]]
- [[y29r3nxgp6q8b8jig2na2lyxf7lr09a6-perl5.38.2-YAML-1.30]]
- [[ypifi4dsg8g2v6rw6a203kcs95sh6y5k-perl5.38.2-Email-Valid-Loose-0.05]]

## 📁 Input Sources

- `/nix/store/380ibq4l01y2bphpsdxp5q9w5szmj3y1-builder.sh`
- `/nix/store/l622p70vy8k5sh7y5wizi5f2mic6ynpg-source-stdenv.sh`

---
*Generated on 2025-09-27 12:16:38 UTC*
