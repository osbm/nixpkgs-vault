---
aliases:
  - heimdall-gui
tags:
  - license/unknown
  - maintainers/surfaceflinger
  - maintainers/timschumi
  - outputs/out
  - outputs/udev
---

# heimdall-gui

## 📝 Description

Cross-platform open-source tool suite used to flash firmware onto Samsung Galaxy devices

## 📋 Package Information

- **Name**: `heimdall-gui`
- **Version**: `2.2.2`
- **Available**: ✅ Yes
- **Broken**: ✅ No
- **Description**: Cross-platform open-source tool suite used to flash firmware onto Samsung Galaxy devices
- **Homepage**: [https://git.sr.ht/~grimler/Heimdall](https://git.sr.ht/~grimler/Heimdall)
- **License**: `unknown`
- **Platforms**: `i686-cygwin`, `x86_64-cygwin`, `x86_64-darwin`, `aarch64-darwin`, `i686-freebsd`, `x86_64-freebsd`, `aarch64-freebsd`, `x86_64-solaris`, `aarch64-linux`, `armv5tel-linux`, `armv6l-linux`, `armv7a-linux`, `armv7l-linux`, `i686-linux`, `loongarch64-linux`, `m68k-linux`, `microblaze-linux`, `microblazeel-linux`, `mips-linux`, `mips64-linux`, `mips64el-linux`, `mipsel-linux`, `powerpc-linux`, `powerpc64-linux`, `powerpc64le-linux`, `riscv32-linux`, `riscv64-linux`, `s390-linux`, `s390x-linux`, `x86_64-linux`, `aarch64-netbsd`, `armv6l-netbsd`, `armv7a-netbsd`, `armv7l-netbsd`, `i686-netbsd`, `m68k-netbsd`, `mipsel-netbsd`, `powerpc-netbsd`, `riscv32-netbsd`, `riscv64-netbsd`, `x86_64-netbsd`, `i686-openbsd`, `x86_64-openbsd`, `x86_64-redox`, `i686-cygwin`, `x86_64-cygwin`, `aarch64-windows`, `x86_64-windows`, `i686-windows`
## 👥 Maintainers

- @surfaceflinger
- @timschumi


## 🔧 Build Information

- **Derivation Path**: `/nix/store/hv92mvr8m430lz0zfiw86aa459spkw6p-heimdall-2.2.2.drv`
- **Source Position**: `/nix/store/ns30sqxb36k8jrds8z18rv96bpnwc60d-source/pkgs/by-name/he/heimdall/package.nix:65`
- **Outputs**:
  - `out`:  `/nix/store/hv92mvr8m430lz0zfiw86aa459spkw6p-heimdall-2.2.2`
  - `udev`:  `/nix/store/hv92mvr8m430lz0zfiw86aa459spkw6p-heimdall-2.2.2`

## 🔗 Dependencies

- [[6rbcy8zva30xk4lm4skl5ykz51vnjqdd-qtbase-6.9.2]]
- [[9j8w4bcicjkza42lizkrrx7sb6jw2qik-cmake-3.31.7]]
- [[bjsb6wdjykafnkixq156qdvmxhsm2bai-bash-5.3p3]]
- [[fy9wjrqf0pzd5x3rvy665v50ff747wfd-libusb-1.0.29]]
- [[ibjx4fb6iqplf03nis4gnwq3yji7h5j7-wrap-qt6-apps-hook]]
- [[lvdvlk7cwad5mna0wfpz8jllb30jdj1n-pkg-config-wrapper-0.29.2]]
- [[p76r0cwlf6k97ibprrpfd8xw0r8wc3nx-stdenv-linux]]
- [[wcc1z6zvxwwqarnj2j554vmdabaxmrin-source]]

## 📁 Input Sources

- `/nix/store/l622p70vy8k5sh7y5wizi5f2mic6ynpg-source-stdenv.sh`
- `/nix/store/shkw4qm9qcw5sc5n1k5jznc83ny02r39-default-builder.sh`
- `/nix/store/ynciig3nl38kzcwngbyz2k68mw22infz-0001-Install-the-macOS-bundle-to-the-install-prefix.patch`

---
*Generated on 2025-09-27 13:11:04 UTC*
