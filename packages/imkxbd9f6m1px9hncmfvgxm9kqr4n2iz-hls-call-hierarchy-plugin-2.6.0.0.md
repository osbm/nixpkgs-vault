---
aliases:
  - haskellPackages.hls-call-hierarchy-plugin
tags:
  - license/unknown
  - outputs/doc
  - outputs/out
---

# haskellPackages.hls-call-hierarchy-plugin

## 📝 Description

Call hierarchy plugin for Haskell Language Server

## 📋 Package Information

- **Name**: `haskellPackages.hls-call-hierarchy-plugin`
- **Version**: `2.6.0.0`
- **Available**: ✅ Yes
- **Broken**: ✅ No
- **Description**: Call hierarchy plugin for Haskell Language Server
- **Homepage**: [https://hackage.haskell.org/package/hls-call-hierarchy-plugin](https://hackage.haskell.org/package/hls-call-hierarchy-plugin)
- **License**: `unknown`
- **Platforms**: `i686-cygwin`, `x86_64-cygwin`, `x86_64-darwin`, `aarch64-darwin`, `i686-freebsd`, `x86_64-freebsd`, `aarch64-freebsd`, `aarch64-genode`, `i686-genode`, `x86_64-genode`, `x86_64-solaris`, `javascript-ghcjs`, `aarch64-linux`, `armv5tel-linux`, `armv6l-linux`, `armv7a-linux`, `armv7l-linux`, `i686-linux`, `loongarch64-linux`, `m68k-linux`, `microblaze-linux`, `microblazeel-linux`, `mips-linux`, `mips64-linux`, `mips64el-linux`, `mipsel-linux`, `powerpc-linux`, `powerpc64-linux`, `powerpc64le-linux`, `riscv32-linux`, `riscv64-linux`, `s390-linux`, `s390x-linux`, `x86_64-linux`, `mmix-mmixware`, `aarch64-netbsd`, `armv6l-netbsd`, `armv7a-netbsd`, `armv7l-netbsd`, `i686-netbsd`, `m68k-netbsd`, `mipsel-netbsd`, `powerpc-netbsd`, `riscv32-netbsd`, `riscv64-netbsd`, `x86_64-netbsd`, `aarch64_be-none`, `aarch64-none`, `arm-none`, `armv6l-none`, `avr-none`, `i686-none`, `microblaze-none`, `microblazeel-none`, `mips-none`, `mips64-none`, `msp430-none`, `or1k-none`, `m68k-none`, `powerpc-none`, `powerpcle-none`, `riscv32-none`, `riscv64-none`, `rx-none`, `s390-none`, `s390x-none`, `vc4-none`, `x86_64-none`, `i686-openbsd`, `x86_64-openbsd`, `x86_64-redox`, `wasm64-wasi`, `wasm32-wasi`, `aarch64-windows`, `x86_64-windows`, `i686-windows`

## 🔧 Build Information

- **Derivation Path**: `/nix/store/imkxbd9f6m1px9hncmfvgxm9kqr4n2iz-hls-call-hierarchy-plugin-2.6.0.0.drv`
- **Source Position**: `/nix/store/ns30sqxb36k8jrds8z18rv96bpnwc60d-source/pkgs/development/haskell-modules/hackage-packages.nix:333771`
- **Outputs**:
  - `doc`:  `/nix/store/imkxbd9f6m1px9hncmfvgxm9kqr4n2iz-hls-call-hierarchy-plugin-2.6.0.0`
  - `out`:  `/nix/store/imkxbd9f6m1px9hncmfvgxm9kqr4n2iz-hls-call-hierarchy-plugin-2.6.0.0`

## 🔗 Dependencies

- [[1rb89fvixwb55ng3b2vrvqcx1j718k3j-ghcide-2.11.0.0]]
- [[49fj7z1vxccad3l5kd67937ssy01k1x0-lens-5.3.5]]
- [[4lykrm96bxajhbrv42nzm365n9yf9s1y-coreutils-9.7]]
- [[4vndlh7irsz90gjyhjx2ikll71vj1cpl-sqlite-simple-0.4.19.0]]
- [[55skdvm9nvfv299nmajgpznd8x6mns9s-glibc-locales-2.40-66]]
- [[bjsb6wdjykafnkixq156qdvmxhsm2bai-bash-5.3p3]]
- [[fww8l3vyg2n6zlh6p8bvhf6r4f76plh8-ghcide-test-utils-2.0.0.0]]
- [[gy9bk0wfd8f2mzzwj5rqbp00adqbl7ph-remove-references-to]]
- [[h3y63zg8n5zllfxxz8njvdy43a0dwg1s-hscolour-1.25]]
- [[ha6nvfy0z5c68jw6af46br6crcnrfz5g-unordered-containers-0.2.20]]
- [[hh7vnqszpw6jls46yskljngxwl2995c6-aeson-2.2.3.0]]
- [[hnym2p5s1bcbb230zhyjfw0yajgd4mmr-lsp-2.7.0.1]]
- [[i1xgs6cdax445491kdkrl9dwkjrk3qjb-haskell-generic-builder-test-wrapper.sh]]
- [[l1ya8rcgk405jscx6136q18v70r0lamx-ghc-9.8.4]]
- [[p76r0cwlf6k97ibprrpfd8xw0r8wc3nx-stdenv-linux]]
- [[q7fw1n41sx73bz6nnxi5q7ihvc4xfbj2-hls-plugin-api-2.11.0.0]]
- [[rcwfsxv9djh7acgnhcidslkiqx5c49sa-extra-1.7.16]]
- [[w5v085kvy56n0fi7dkr6ga3j943pmxsr-hiedb-0.6.0.2]]
- [[wdf2hg8213zf5xpvgbzzyv62vhx9jn66-hls-test-utils-2.11.0.0]]
- [[wx2y17fy4syjhqgzmv0mdli4s2mpgjzl-lsp-test-0.17.1.1]]
- [[y0j83qbssw99mm0h3m6qan8d2f4rf4ly-hls-call-hierarchy-plugin-2.6.0.0.tar.gz]]

## 📁 Input Sources

- `/nix/store/4mdp8nhyfddh7bllbi7xszz7k9955n79-Setup.hs`
- `/nix/store/907d7wf2d113vlv8dlhi1i3d683gc1rs-unpretty-cabal-conf.awk`
- `/nix/store/l622p70vy8k5sh7y5wizi5f2mic6ynpg-source-stdenv.sh`
- `/nix/store/shkw4qm9qcw5sc5n1k5jznc83ny02r39-default-builder.sh`

---
*Generated on 2025-09-27 12:17:07 UTC*
