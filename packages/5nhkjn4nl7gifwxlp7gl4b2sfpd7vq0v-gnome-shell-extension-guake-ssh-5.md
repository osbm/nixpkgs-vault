---
aliases:
  - gnomeExtensions.guake-ssh
tags:
  - license/unknown
  - maintainers/honnip
  - outputs/out
---

# gnomeExtensions.guake-ssh

## 📝 Description

Systray menu to connect to SSH hosts configured in ~/.ssh/config or any files in  ~/.ssh/config.d/ using Guake.

- Left mouse click: new tab
- Right mouse click: split vertical current tab
- Middle mouse click: split horizontal current tab

More information on the extension homepage.

## 📋 Package Information

- **Name**: `gnomeExtensions.guake-ssh`
- **Version**: `5`
- **Available**: ✅ Yes
- **Broken**: ✅ No
- **Description**: Systray menu to connect to SSH hosts configured in ~/.ssh/config or any files in  ~/.ssh/config.d/ using Guake.
- **Homepage**: [https://extensions.gnome.org/extension/8012/guake-ssh/](https://extensions.gnome.org/extension/8012/guake-ssh/)
- **License**: `unknown`
- **Platforms**: `aarch64-linux`, `armv5tel-linux`, `armv6l-linux`, `armv7a-linux`, `armv7l-linux`, `i686-linux`, `loongarch64-linux`, `m68k-linux`, `microblaze-linux`, `microblazeel-linux`, `mips-linux`, `mips64-linux`, `mips64el-linux`, `mipsel-linux`, `powerpc-linux`, `powerpc64-linux`, `powerpc64le-linux`, `riscv32-linux`, `riscv64-linux`, `s390-linux`, `s390x-linux`, `x86_64-linux`
## 👥 Maintainers

- @honnip


## 🔧 Build Information

- **Derivation Path**: `/nix/store/5nhkjn4nl7gifwxlp7gl4b2sfpd7vq0v-gnome-shell-extension-guake-ssh-5.drv`
- **Source Position**: `/nix/store/ns30sqxb36k8jrds8z18rv96bpnwc60d-source/pkgs/desktops/gnome/extensions/buildGnomeExtension.nix:61`
- **Outputs**:
  - `out`:  `/nix/store/5nhkjn4nl7gifwxlp7gl4b2sfpd7vq0v-gnome-shell-extension-guake-ssh-5`

## 🔗 Dependencies

- [[bjsb6wdjykafnkixq156qdvmxhsm2bai-bash-5.3p3]]
- [[glca1gx472ps9qlivbdf7z5jdcdnsp9l-glib-2.84.4]]
- [[lsf71r5s7j2mvvaapc7c54y7vzqigaxx-source]]
- [[p76r0cwlf6k97ibprrpfd8xw0r8wc3nx-stdenv-linux]]

## 📁 Input Sources

- `/nix/store/l622p70vy8k5sh7y5wizi5f2mic6ynpg-source-stdenv.sh`
- `/nix/store/shkw4qm9qcw5sc5n1k5jznc83ny02r39-default-builder.sh`

---
*Generated on 2025-09-27 12:15:56 UTC*
