---
aliases:
  - haskellPackages.hw-xml
tags:
  - broken
  - not-available
  - license/unknown
  - outputs/data
  - outputs/doc
  - outputs/out
---

# haskellPackages.hw-xml

## 📝 Description

XML parser based on succinct data structures

## 📋 Package Information

- **Name**: `haskellPackages.hw-xml`
- **Version**: `0.5.1.2`
- **Available**: ❌ No
- **Broken**: ⚠️ Yes
- **Description**: XML parser based on succinct data structures
- **Homepage**: [https://hackage.haskell.org/package/hw-xml](https://hackage.haskell.org/package/hw-xml)
- **License**: `unknown`
- **Platforms**: `i686-cygwin`, `x86_64-cygwin`, `x86_64-darwin`, `aarch64-darwin`, `i686-freebsd`, `x86_64-freebsd`, `aarch64-freebsd`, `aarch64-genode`, `i686-genode`, `x86_64-genode`, `x86_64-solaris`, `javascript-ghcjs`, `aarch64-linux`, `armv5tel-linux`, `armv6l-linux`, `armv7a-linux`, `armv7l-linux`, `i686-linux`, `loongarch64-linux`, `m68k-linux`, `microblaze-linux`, `microblazeel-linux`, `mips-linux`, `mips64-linux`, `mips64el-linux`, `mipsel-linux`, `powerpc-linux`, `powerpc64-linux`, `powerpc64le-linux`, `riscv32-linux`, `riscv64-linux`, `s390-linux`, `s390x-linux`, `x86_64-linux`, `mmix-mmixware`, `aarch64-netbsd`, `armv6l-netbsd`, `armv7a-netbsd`, `armv7l-netbsd`, `i686-netbsd`, `m68k-netbsd`, `mipsel-netbsd`, `powerpc-netbsd`, `riscv32-netbsd`, `riscv64-netbsd`, `x86_64-netbsd`, `aarch64_be-none`, `aarch64-none`, `arm-none`, `armv6l-none`, `avr-none`, `i686-none`, `microblaze-none`, `microblazeel-none`, `mips-none`, `mips64-none`, `msp430-none`, `or1k-none`, `m68k-none`, `powerpc-none`, `powerpcle-none`, `riscv32-none`, `riscv64-none`, `rx-none`, `s390-none`, `s390x-none`, `vc4-none`, `x86_64-none`, `i686-openbsd`, `x86_64-openbsd`, `x86_64-redox`, `wasm64-wasi`, `wasm32-wasi`, `aarch64-windows`, `x86_64-windows`, `i686-windows`

## 🔧 Build Information

- **Derivation Path**: `/nix/store/a02ygjjv7jb9cyw7a0xcm5fh3bi2y5gy-hw-xml-0.5.1.2.drv`
- **Source Position**: `/nix/store/ns30sqxb36k8jrds8z18rv96bpnwc60d-source/pkgs/development/haskell-modules/hackage-packages.nix:368581`
- **Outputs**:
  - `data`:  `/nix/store/a02ygjjv7jb9cyw7a0xcm5fh3bi2y5gy-hw-xml-0.5.1.2`
  - `doc`:  `/nix/store/a02ygjjv7jb9cyw7a0xcm5fh3bi2y5gy-hw-xml-0.5.1.2`
  - `out`:  `/nix/store/a02ygjjv7jb9cyw7a0xcm5fh3bi2y5gy-hw-xml-0.5.1.2`

## 🔗 Dependencies

- [[0vgxxmadx9q0nd5p4mmbm85yjnbrgbj2-hw-hspec-hedgehog-0.1.1.1]]
- [[0yrz11qijjkr0bd3s76dfbdvgn2inmlg-hspec-discover-2.11.12]]
- [[15l0pvkj8iqapppacyx83xh2k409nz34-hw-prim-0.6.3.2]]
- [[45wky4h8vdpjfgbfywky6wd460dcnk82-hspec-2.11.12]]
- [[49fj7z1vxccad3l5kd67937ssy01k1x0-lens-5.3.5]]
- [[4lykrm96bxajhbrv42nzm365n9yf9s1y-coreutils-9.7]]
- [[55skdvm9nvfv299nmajgpznd8x6mns9s-glibc-locales-2.40-66]]
- [[6aagal11rbj713rwnpi70k9d7qbq5h4g-hw-rankselect-base-0.3.4.1]]
- [[6jy98h01lwbyj6pbcfbkakbfw6c7al8a-cereal-0.5.8.3]]
- [[8hmdccc8v4fvkdzsq8lqz8wq936m4nki-optparse-applicative-0.18.1.0]]
- [[8qkd7kpmmanfg0029vj05djkq5lkq2w3-hw-rankselect-0.13.4.1]]
- [[97v1vbllm5qig6rmanb6flalp39h41yg-doctest-discover-0.2.0.0]]
- [[bjsb6wdjykafnkixq156qdvmxhsm2bai-bash-5.3p3]]
- [[d9k9q5l0567l4x55ssdp8hsxahibl3ja-generic-lens-2.2.2.0]]
- [[gy9bk0wfd8f2mzzwj5rqbp00adqbl7ph-remove-references-to]]
- [[h3y63zg8n5zllfxxz8njvdy43a0dwg1s-hscolour-1.25]]
- [[i1xgs6cdax445491kdkrl9dwkjrk3qjb-haskell-generic-builder-test-wrapper.sh]]
- [[l1ya8rcgk405jscx6136q18v70r0lamx-ghc-9.8.4]]
- [[m4w470jkqi4l005rbfyvsczkkdw19j84-hw-balancedparens-0.4.1.3]]
- [[n8g7rkf98pxzd7ak9kw2ma40h643dfsg-attoparsec-0.14.4]]
- [[ngmn5kanhwzy43ms7g8a1qn44iqnp4zr-mmap-0.5.9]]
- [[p76r0cwlf6k97ibprrpfd8xw0r8wc3nx-stdenv-linux]]
- [[q7mnhh1p6gf7mvwy8ckk88hlwaq52h66-hw-parser-0.1.1.0]]
- [[r2rfh31xr9w4nv8a2ll6xhfl7fh0dfn9-hedgehog-1.5]]
- [[r82q6wv6jad8qc7ckdkziylbin3hig3j-hw-xml-0.5.1.2.tar.gz]]
- [[v9a84jfph4k9x7yhhf2zmygw5k3q0732-vector-0.13.2.0]]
- [[vicrbcq3rm5sj39dah5dp5bvkmzgwjbv-hw-bits-0.7.2.2]]
- [[w8ha91k8lxpbjq0vvc84zmib5xz5id3c-resourcet-1.3.0]]
- [[y1z595j4ca34zgmyxa78qna39c40v9fl-word8-0.1.3]]
- [[ywxx36yrch0b8xygw5gr0n9zsbaqm2nm-doctest-0.22.6]]
- [[zajhzr74wicxg5sjn22039r97rjzii0n-prettyprinter-1.7.1]]

## 📁 Input Sources

- `/nix/store/4mdp8nhyfddh7bllbi7xszz7k9955n79-Setup.hs`
- `/nix/store/907d7wf2d113vlv8dlhi1i3d683gc1rs-unpretty-cabal-conf.awk`
- `/nix/store/l622p70vy8k5sh7y5wizi5f2mic6ynpg-source-stdenv.sh`
- `/nix/store/shkw4qm9qcw5sc5n1k5jznc83ny02r39-default-builder.sh`

---
*Generated on 2025-09-27 12:03:58 UTC*
