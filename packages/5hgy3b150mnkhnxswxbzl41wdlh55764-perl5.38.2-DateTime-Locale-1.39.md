---
aliases:
  - perl538Packages.DateTimeLocale
tags:
  - license/unknown
  - outputs/devdoc
  - outputs/out
---

# perl538Packages.DateTimeLocale

## 📝 Description

Localization support for DateTime.pm

## 📋 Package Information

- **Name**: `perl538Packages.DateTimeLocale`
- **Version**: `1.39`
- **Available**: ✅ Yes
- **Broken**: ✅ No
- **Description**: Localization support for DateTime.pm
- **Homepage**: [https://metacpan.org/release/DateTime-Locale](https://metacpan.org/release/DateTime-Locale)
- **License**: `unknown`
- **Platforms**: `i686-cygwin`, `x86_64-cygwin`, `x86_64-darwin`, `aarch64-darwin`, `i686-freebsd`, `x86_64-freebsd`, `aarch64-freebsd`, `aarch64-genode`, `i686-genode`, `x86_64-genode`, `x86_64-solaris`, `javascript-ghcjs`, `aarch64-linux`, `armv5tel-linux`, `armv6l-linux`, `armv7a-linux`, `armv7l-linux`, `i686-linux`, `loongarch64-linux`, `m68k-linux`, `microblaze-linux`, `microblazeel-linux`, `mips-linux`, `mips64-linux`, `mips64el-linux`, `mipsel-linux`, `powerpc-linux`, `powerpc64-linux`, `powerpc64le-linux`, `riscv32-linux`, `riscv64-linux`, `s390-linux`, `s390x-linux`, `x86_64-linux`, `mmix-mmixware`, `aarch64-netbsd`, `armv6l-netbsd`, `armv7a-netbsd`, `armv7l-netbsd`, `i686-netbsd`, `m68k-netbsd`, `mipsel-netbsd`, `powerpc-netbsd`, `riscv32-netbsd`, `riscv64-netbsd`, `x86_64-netbsd`, `aarch64_be-none`, `aarch64-none`, `arm-none`, `armv6l-none`, `avr-none`, `i686-none`, `microblaze-none`, `microblazeel-none`, `mips-none`, `mips64-none`, `msp430-none`, `or1k-none`, `m68k-none`, `powerpc-none`, `powerpcle-none`, `riscv32-none`, `riscv64-none`, `rx-none`, `s390-none`, `s390x-none`, `vc4-none`, `x86_64-none`, `i686-openbsd`, `x86_64-openbsd`, `x86_64-redox`, `wasm64-wasi`, `wasm32-wasi`, `aarch64-windows`, `x86_64-windows`, `i686-windows`

## 🔧 Build Information

- **Derivation Path**: `/nix/store/5hgy3b150mnkhnxswxbzl41wdlh55764-perl5.38.2-DateTime-Locale-1.39.drv`
- **Source Position**: `/nix/store/ns30sqxb36k8jrds8z18rv96bpnwc60d-source/pkgs/top-level/perl-packages.nix:9295`
- **Outputs**:
  - `devdoc`:  `/nix/store/5hgy3b150mnkhnxswxbzl41wdlh55764-perl5.38.2-DateTime-Locale-1.39`
  - `out`:  `/nix/store/5hgy3b150mnkhnxswxbzl41wdlh55764-perl5.38.2-DateTime-Locale-1.39`

## 🔗 Dependencies

- [[2l3a6inqkpzy4hvqgz94qw7rnc8slnbq-perl5.38.2-namespace-autoclean-0.29]]
- [[4w6dzhzmzn34hravxgb351mkbbpi5n5k-perl-5.38.2]]
- [[5d1y9h40q3c2l9l3afhmb3wvyi5sm2n6-perl5.38.2-Params-ValidationCompiler-0.31]]
- [[69fmkl6z2pp4dvb879hvqxyiwv5ba4hl-perl5.38.2-Specio-0.48]]
- [[6lkn4fcj1mxsy6809kkh8cxlrxbabfxp-perl5.38.2-File-ShareDir-1.118]]
- [[76f1i2c5wbbafssrsi0yj4hcb0dqck1q-perl5.38.2-Path-Tiny-0.144]]
- [[7i5fkadimzrxk4b4sy9l7lgah3m1kqbq-perl5.38.2-Test2-Suite-0.000156]]
- [[7ws1ww7bw7psqqpdccz9l8r985dxnn6p-perl5.38.2-Test2-Plugin-NoWarnings-0.09]]
- [[bjsb6wdjykafnkixq156qdvmxhsm2bai-bash-5.3p3]]
- [[farnrav4nbld2bcz7lh446h8cff3whja-perl5.38.2-File-ShareDir-Install-0.14]]
- [[j9b47i2bi3l2gjlal4bkgkrz17z1n2q7-perl5.38.2-Test-File-ShareDir-1.001002]]
- [[p1hvl5zb56n2g3v4kix4d5ih7laxsmfy-DateTime-Locale-1.39.tar.gz]]
- [[p76r0cwlf6k97ibprrpfd8xw0r8wc3nx-stdenv-linux]]
- [[pwmdkjjkybw3gga3z1yd73rc75a0z0g4-perl5.38.2-IPC-System-Simple-1.30]]
- [[x2swg5a0d4s0xli51w53ci96iqima5r5-perl5.38.2-CPAN-Meta-Check-0.018]]

## 📁 Input Sources

- `/nix/store/380ibq4l01y2bphpsdxp5q9w5szmj3y1-builder.sh`
- `/nix/store/l622p70vy8k5sh7y5wizi5f2mic6ynpg-source-stdenv.sh`

---
*Generated on 2025-09-27 12:15:03 UTC*
