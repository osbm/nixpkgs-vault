---
aliases:
  - tuxedo-rs
tags:
  - license/unknown
  - maintainers/mrcjkb
  - maintainers/xaverdh
  - outputs/out
---

# tuxedo-rs

## 📝 Description

An alternative to the TUXEDO Control Center daemon.

Contains the following binaries:
- tailord: Daemon handling fan, keyboard and general HW support for Tuxedo laptops
- tailor: CLI


## 📋 Package Information

- **Name**: `tuxedo-rs`
- **Version**: `0.3.1`
- **Available**: ✅ Yes
- **Broken**: ✅ No
- **Description**: Rust utilities for interacting with hardware from TUXEDO Computers
- **Homepage**: [https://github.com/AaronErhardt/tuxedo-rs](https://github.com/AaronErhardt/tuxedo-rs)
- **License**: `unknown`
- **Platforms**: `aarch64-linux`, `armv6l-linux`, `armv7l-linux`, `i686-linux`, `loongarch64-linux`, `powerpc-linux`, `powerpc64-linux`, `powerpc64le-linux`, `riscv64-linux`, `s390x-linux`, `x86_64-linux`, `armv5tel-linux`, `armv7a-linux`, `m68k-linux`, `mips-linux`, `mips64-linux`, `mipsel-linux`, `mips64el-linux`, `riscv32-linux`
## 👥 Maintainers

- @mrcjkb
- @xaverdh


## 🔧 Build Information

- **Derivation Path**: `/nix/store/xc43iikb4y2c29k8mzdfw7k1sbzwjw1c-tuxedo-rs-0.3.1.drv`
- **Source Position**: `/nix/store/ns30sqxb36k8jrds8z18rv96bpnwc60d-source/pkgs/by-name/tu/tuxedo-rs/package.nix:36`
- **Outputs**:
  - `out`:  `/nix/store/xc43iikb4y2c29k8mzdfw7k1sbzwjw1c-tuxedo-rs-0.3.1`

## 🔗 Dependencies

- [[0f5ann1sgwyp31vlfqqsnr95xzv3mzhf-cargo-1.89.0]]
- [[3bz4j2xzjl92yhiwfyhbz6gv4w1888bw-cargo-setup-hook.sh]]
- [[3ndchkhlxxw50k7nd48i9zdkhsr57mwh-cargo-install-hook.sh]]
- [[4bycw2rdgf33gmn1z26knv8grxxh06w5-rustc-wrapper-1.89.0]]
- [[4cc0y6syxyxvjx1lh8ix60vxkly149fh-auditable-cargo-1.89.0]]
- [[4nij599rw4r7nnav7framfj4wwa8cr4i-tuxedo-rs-0.3.1-vendor]]
- [[74q0nywxj0f82imn18hcnccrwr2aj653-source]]
- [[bjsb6wdjykafnkixq156qdvmxhsm2bai-bash-5.3p3]]
- [[p76r0cwlf6k97ibprrpfd8xw0r8wc3nx-stdenv-linux]]
- [[qi6337yh29r9ppqm262bf6vz0g13451b-cargo-build-hook.sh]]
- [[xqxbqs6qgnhvpkcai046s94jfrx8wb62-cargo-check-hook.sh]]

## 📁 Input Sources

- `/nix/store/l622p70vy8k5sh7y5wizi5f2mic6ynpg-source-stdenv.sh`
- `/nix/store/shkw4qm9qcw5sc5n1k5jznc83ny02r39-default-builder.sh`

---
*Generated on 2025-09-27 12:57:28 UTC*
