---
aliases:
  - lua53Packages.fzf-lua
tags:
  - license/unknown
  - maintainers/mrcjkb
  - outputs/out
---

# lua53Packages.fzf-lua

## 📝 Description

Improved fzf.vim written in lua

## 📋 Package Information

- **Name**: `lua53Packages.fzf-lua`
- **Version**: `0.0.2234-1`
- **Available**: ✅ Yes
- **Broken**: ✅ No
- **Description**: Improved fzf.vim written in lua
- **Homepage**: [https://github.com/ibhagwan/fzf-lua](https://github.com/ibhagwan/fzf-lua)
- **License**: `unknown`
- **Platforms**: `i686-cygwin`, `x86_64-cygwin`, `x86_64-darwin`, `aarch64-darwin`, `i686-freebsd`, `x86_64-freebsd`, `aarch64-freebsd`, `x86_64-solaris`, `aarch64-linux`, `armv5tel-linux`, `armv6l-linux`, `armv7a-linux`, `armv7l-linux`, `i686-linux`, `loongarch64-linux`, `m68k-linux`, `microblaze-linux`, `microblazeel-linux`, `mips-linux`, `mips64-linux`, `mips64el-linux`, `mipsel-linux`, `powerpc-linux`, `powerpc64-linux`, `powerpc64le-linux`, `riscv32-linux`, `riscv64-linux`, `s390-linux`, `s390x-linux`, `x86_64-linux`, `aarch64-netbsd`, `armv6l-netbsd`, `armv7a-netbsd`, `armv7l-netbsd`, `i686-netbsd`, `m68k-netbsd`, `mipsel-netbsd`, `powerpc-netbsd`, `riscv32-netbsd`, `riscv64-netbsd`, `x86_64-netbsd`, `i686-openbsd`, `x86_64-openbsd`, `x86_64-redox`
## 👥 Maintainers

- @mrcjkb


## 🔧 Build Information

- **Derivation Path**: `/nix/store/1jlw2rlxqsjix37709d5p3s4spnypx3n-lua5.3-fzf-lua-0.0.2234-1.drv`
- **Source Position**: `/nix/store/ns30sqxb36k8jrds8z18rv96bpnwc60d-source/pkgs/development/lua-modules/generated-packages.nix:832`
- **Outputs**:
  - `out`:  `/nix/store/1jlw2rlxqsjix37709d5p3s4spnypx3n-lua5.3-fzf-lua-0.0.2234-1`

## 🔗 Dependencies

- [[3g2sjr8sdkrlvqh8fb4wv9qddm7gym7p-fzf-lua-luarocks-config.lua]]
- [[bjsb6wdjykafnkixq156qdvmxhsm2bai-bash-5.3p3]]
- [[cmg15axq5p26x8sqlpvk3yba80w91rl2-vimplugin-luajit2.1-nvim-web-devicons-0.100-1-unstable-0.100-1]]
- [[cs9acw9ya6b5k7r1c4c01j3z5mc5rr0d-luarocks_bootstrap-3.12.2]]
- [[knbfrkni2clbghxjf1m5wlm27d6l8fvk-wrap-lua-hook]]
- [[p76r0cwlf6k97ibprrpfd8xw0r8wc3nx-stdenv-linux]]
- [[qzjhzzb0nnn8j5npda7659d980y3zs8x-source]]
- [[x522hrrnbvq1f9r5pm4bl5rdl463rpck-fzf-lua-0.0.2234-1.rockspec]]
- [[y19ag8kvzll9xvx160r2qw32bl1qb0iw-vimplugin-mini.nvim-2025-09-14]]
- [[zkcwcs4firjwjzv66kbvy3i1zpik10p3-lua-5.3.6]]

## 📁 Input Sources

- `/nix/store/l622p70vy8k5sh7y5wizi5f2mic6ynpg-source-stdenv.sh`
- `/nix/store/shkw4qm9qcw5sc5n1k5jznc83ny02r39-default-builder.sh`

---
*Generated on 2025-09-27 11:58:38 UTC*
