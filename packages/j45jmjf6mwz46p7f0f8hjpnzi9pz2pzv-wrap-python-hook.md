---
aliases:
  - python312Packages.wrapPython
tags:
  - license/unknown
  - outputs/out
---

# python312Packages.wrapPython

## 📋 Package Information

- **Name**: `python312Packages.wrapPython`
- **Available**: ✅ Yes
- **Broken**: ✅ No
- **License**: `unknown`

## 🔧 Build Information

- **Derivation Path**: `/nix/store/j45jmjf6mwz46p7f0f8hjpnzi9pz2pzv-wrap-python-hook.drv`
- **Source Position**: `/nix/store/ns30sqxb36k8jrds8z18rv96bpnwc60d-source/pkgs/build-support/trivial-builders/default.nix:80`
- **Outputs**:
  - `out`:  `/nix/store/j45jmjf6mwz46p7f0f8hjpnzi9pz2pzv-wrap-python-hook`

## 🔗 Dependencies

- [[2jbjfm0s7c03a7mylffys7n228vs64rz-make-shell-wrapper-hook]]
- [[6dg1vi55ynf4dmkmmcn945pwdz010s34-stdenv-linux]]
- [[bjsb6wdjykafnkixq156qdvmxhsm2bai-bash-5.3p3]]
- [[fz0k8m7chhichwdj1h1g54hjfh80g3nm-python3-3.12.11]]

## 📁 Input Sources

- `/nix/store/ag815hbmmkgk4flh7rj3nw90b45kik64-wrap.sh`
- `/nix/store/l622p70vy8k5sh7y5wizi5f2mic6ynpg-source-stdenv.sh`
- `/nix/store/shkw4qm9qcw5sc5n1k5jznc83ny02r39-default-builder.sh`

---
*Generated on 2025-09-27 12:17:57 UTC*
