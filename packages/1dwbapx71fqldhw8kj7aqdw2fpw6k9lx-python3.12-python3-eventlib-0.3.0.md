---
aliases:
  - python312Packages.python3-eventlib
tags:
  - license/unknown
  - maintainers/charlieshanley
  - outputs/dist
  - outputs/out
---

# python312Packages.python3-eventlib

## 📝 Description

Eventlib is a networking library written in Python. It achieves high
scalability by using non-blocking I/O while at the same time retaining
high programmer usability by using coroutines to make the non-blocking io
operations appear blocking at the source code level.


## 📋 Package Information

- **Name**: `python312Packages.python3-eventlib`
- **Version**: `0.3.0`
- **Available**: ✅ Yes
- **Broken**: ✅ No
- **Description**: Networking library written in Python
- **Homepage**: [https://github.com/AGProjects/python3-eventlib](https://github.com/AGProjects/python3-eventlib)
- **License**: `unknown`
- **Platforms**: `aarch64-linux`, `armv5tel-linux`, `armv6l-linux`, `armv7a-linux`, `armv7l-linux`, `i686-linux`, `loongarch64-linux`, `m68k-linux`, `microblaze-linux`, `microblazeel-linux`, `mips-linux`, `mips64-linux`, `mips64el-linux`, `mipsel-linux`, `powerpc-linux`, `powerpc64-linux`, `powerpc64le-linux`, `riscv32-linux`, `riscv64-linux`, `s390-linux`, `s390x-linux`, `x86_64-linux`, `x86_64-darwin`, `aarch64-darwin`, `i686-cygwin`, `x86_64-cygwin`, `aarch64-windows`, `x86_64-windows`, `i686-windows`, `i686-freebsd`, `x86_64-freebsd`, `aarch64-freebsd`
## 👥 Maintainers

- @charlieshanley


## 🔧 Build Information

- **Derivation Path**: `/nix/store/1dwbapx71fqldhw8kj7aqdw2fpw6k9lx-python3.12-python3-eventlib-0.3.0.drv`
- **Source Position**: `/nix/store/ns30sqxb36k8jrds8z18rv96bpnwc60d-source/pkgs/development/python-modules/python3-eventlib/default.nix:34`
- **Outputs**:
  - `dist`:  `/nix/store/1dwbapx71fqldhw8kj7aqdw2fpw6k9lx-python3.12-python3-eventlib-0.3.0`
  - `out`:  `/nix/store/1dwbapx71fqldhw8kj7aqdw2fpw6k9lx-python3.12-python3-eventlib-0.3.0`

## 🔗 Dependencies

- [[7vyvc6rdpjf65mrbzbgcax3bp1iz3xf6-setuptools-build-hook]]
- [[96p9l0mp0cy12s10705rpz96x6bgz371-python-output-dist-hook]]
- [[a6avf2bw7091v52d5ddpq6zb792f9q2c-python3.12-zope-interface-7.2]]
- [[b2gcqyf6wr8k19l1h57cgybfm7plixkq-python-remove-bin-bytecode-hook]]
- [[bjsb6wdjykafnkixq156qdvmxhsm2bai-bash-5.3p3]]
- [[di2np59yg9yf560yms5ff9188a5gnlbb-python-imports-check-hook.sh]]
- [[dkqa3dj2k1vqrmlyd6hrdf8dww0f5dr9-python-remove-tests-dir-hook]]
- [[fhrpbbzpqw4bvsy4ixq1fb9k2aaslnw0-python-catch-conflicts-hook]]
- [[fz0k8m7chhichwdj1h1g54hjfh80g3nm-python3-3.12.11]]
- [[g9bcdx47nd5qfi29d66nbxbwckd5g99m-python-namespaces-hook.sh]]
- [[h1zzsmaws1g6lvycvd7p54yajzicplhl-source]]
- [[j45jmjf6mwz46p7f0f8hjpnzi9pz2pzv-wrap-python-hook]]
- [[kh0627w4wff8syd1iis567224170xw3l-pypa-install-hook]]
- [[p76r0cwlf6k97ibprrpfd8xw0r8wc3nx-stdenv-linux]]
- [[yfa3029l77w1aj0pgpkr1mrx4xdmhfhx-python3.12-greenlet-3.2.3]]
- [[z1z8hhwp9iridwshz4vz9pzz7fnpdpph-python3.12-twisted-25.5.0]]
- [[z99vzf35iinh3dnh2g994wbcbjrv4siq-ensure-newer-sources-hook]]

## 📁 Input Sources

- `/nix/store/l622p70vy8k5sh7y5wizi5f2mic6ynpg-source-stdenv.sh`
- `/nix/store/shkw4qm9qcw5sc5n1k5jznc83ny02r39-default-builder.sh`

---
*Generated on 2025-09-27 12:06:47 UTC*
